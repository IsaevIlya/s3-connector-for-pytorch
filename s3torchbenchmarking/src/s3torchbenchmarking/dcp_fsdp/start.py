...
def create_regex_sort_key(regex_container: VirtualRegexContainer) -> Callable:
    def sort_key(item):
        # First priority: matches regex (False sorts after True)
        matches_regex = item.index.fqn in regex_container
        # Second priority: alphabetical order
        return (not matches_regex, item.index.fqn)
    return sort_key
...
def get_writer(region:str, uri: str, suffix: str, thread_count: int = 8, regex_container: VirtualRegexContainer = None) -> FileSystemWriter:
    uri = build_checkpoint_uri(uri, suffix)
    logger.info("Saving checkpoint to %s (S3)...", uri)
    sort_key = None
    if regex_container:
        sort_key = create_regex_sort_key(regex_container)
    return S3StorageWriter(region, uri, thread_count=thread_count, sort_key=sort_key)
...
def run_fsdp(
    ...
    if do_save:
        ...
        regex = VirtualRegexContainer("^model\\.model\\.layers\\.[13][13579]")
        storage_writer = get_writer(region, uri, suffix, thread_count, regex_container=regex)
...
if __name__ == "__main__":
    ...
    run_fsdp(rank, world_size, thread_count, backend, region, uri, suffix,
